...
import com.play.Tashan.Comments.Comment_Get_Set;
...
public class Functions {
    ...
    public static void Call_Api_For_like_video(final Activity activity,
                                               String video_id,String id,String likes,String action) {


        int heartt=Integer.parseInt(likes);
        DatabaseReference mDatabase = FirebaseDatabase.getInstance().getReference("users").child(id).child("uploads").child(video_id).child("likedUsers");
        Log.e("Aniket",id+"video id"+heartt);
        if(action.equals("1")) {
            FirebaseDatabase.getInstance().getReference("users").child(id).child("total_heart").setValue(heartt);
            mDatabase.child(Variables.sharedPreferences.getString(Variables.u_id, "0")).setValue(action);
            FirebaseDatabase.getInstance().getReference("users").child(Variables.sharedPreferences.getString(Variables.u_id, "0")).child("Liked_Videos").child(video_id).setValue(id);
        }
        else {
            mDatabase.child(Variables.sharedPreferences.getString(Variables.u_id, "0")).removeValue();
            FirebaseDatabase.getInstance().getReference("users").child(id).child("total_heart").setValue(heartt);
            FirebaseDatabase.getInstance().getReference("users").child(Variables.sharedPreferences.getString(Variables.u_id, "0")).child("Liked_Videos").child(video_id).removeValue();
        }
    ...
    public static void Call_Api_For_Send_Comment(final Activity activity, final String user_id, final String video_id, final String comment , final API_CallBack api_callBack) {
        DatabaseReference mDatabase=FirebaseDatabase.getInstance().getReference("users").child(user_id);
        SimpleDateFormat sdf =   new SimpleDateFormat("dd-mm-yyyy hh-mm-ss", Locale.getDefault());
        final String created =sdf.format(new Date());
        mDatabase.child("uploads").child(video_id).child("comments").child(Variables.sharedPreferences.getString(Variables.u_id,"")).child("comment").setValue(comment);
        mDatabase.child("uploads").child(video_id).child("comments").child(Variables.sharedPreferences.getString(Variables.u_id,"")).child("created").setValue(created);
        mDatabase.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                try {
                    ...
                    item.fb_id=Variables.sharedPreferences.getString(Variables.u_id,"");

                    //JSONObject user_info=itemdata.optJSONObject("user_info");
                    item.first_name=Variables.sharedPreferences.getString(Variables.f_name,"");
                    Log.e("aloo",item.first_name);
                    item.last_name=Variables.sharedPreferences.getString(Variables.l_name,"");
                    item.profile_pic=Variables.sharedPreferences.getString(Variables.u_pic,"");
                    DataSnapshot postSnapshot=snapshot.child("uploads").child(video_id);
    ...
    public static void Call_Api_For_get_Comment(final Activity activity, final String user_id, final String video_id, final API_CallBack api_callBack) {
        DatabaseReference mDatabase=FirebaseDatabase.getInstance().getReference("users");;
        mDatabase.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ArrayList<Comment_Get_Set> arrayList=new ArrayList<>();
                for (DataSnapshot postSnapshot: snapshot.child(user_id).child("uploads").child(video_id).child("comments").getChildren()) {
                    Comment_Get_Set item=new Comment_Get_Set();
                    item.fb_id=""+postSnapshot.getKey().toString();


                    item.first_name=snapshot.child(item.fb_id).child("first_name").getValue().toString();
                    item.last_name=snapshot.child(item.fb_id).child("last_name").getValue().toString();
                    item.profile_pic=snapshot.child(item.fb_id).child("profile_pic").getValue().toString();


                    item.video_id=video_id;
                    item.comments=""+postSnapshot.child("comment").getValue().toString();
                    item.created=""+postSnapshot.child("created").getValue().toString();
    ...
    public static void Call_Api_For_update_view(final Activity activity,
                                               String video_id,String Views,String id) {
        int views=0;
        if(Views!=null) {
             views = Integer.valueOf(Views);
            views++;
        }

        FirebaseDatabase.getInstance().getReference("users").child(id).child("uploads").child(video_id).child("views").setValue(views);
    ...
    public static void Call_Api_For_Follow_or_unFollow
        ...
        DatabaseReference mDatabase = FirebaseDatabase.getInstance().getReference("users").child(fb_id).child("following");
        Log.e("follow",status);
        if(status.equals("1")) {
            mDatabase.child(followed_fb_id).setValue(status);
            FirebaseDatabase.getInstance().getReference("users").child(followed_fb_id).child("followers").child(fb_id).setValue(status);
        }
        else {
            mDatabase.child(followed_fb_id).removeValue();
            FirebaseDatabase.getInstance().getReference("users").child(followed_fb_id).child("followers").child(fb_id).removeValue();
        }
